...
public: static const int N = Cov1::N + Cov2::N - 1; // one sigma_f
...
protected:
	...
	{
		// copy except the last hyperparameter
		int j = 0;
		for(int i = 0; i < logHyp1.size()-1; i++)	logHyp1(i) = logHyp(j++);
		for(int i = 0; i < logHyp2.size()-1; i++)	logHyp2(i) = logHyp(j++);

		// last hyperparameter: sigma_f
		logHyp1(logHyp1.size()-1) = logHyp(logHyp.size()-1);
		logHyp2(logHyp2.size()-1) = logHyp(logHyp.size()-1);
	}
